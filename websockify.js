...
var new_client = function(client) {
	...
	if(client.upgradeReq && client.upgradeReq.url){
...
var http_request = function (argv, request, response) {
...
function initWsServer(_argv) {
	...
	if (argv.cert) {
		...
		webServer = https.createServer({cert: cert, key: key}, http_request.bind(null, argv));
	} else {
		console.log("    - Running in unencrypted HTTP (ws://) mode");
		webServer = http.createServer(http_request.bind(null, argv));
